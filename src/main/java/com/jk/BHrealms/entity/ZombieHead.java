...
public class ZombieHead extends Monster implements SmartBrainOwner<ZombieHead>, GeoEntity {
    public static final EntityDataAccessor<Boolean> TAUNTED = SynchedEntityData.defineId(ZombieHead.class, EntityDataSerializers.BOOLEAN);
    ...
    public ZombieHead(EntityType<? extends ZombieHead> entityType, Level level) {
        super(entityType, level);
        this.setMaxUpStep(1.25f);

    }

    public ZombieHead(Level level, double x, double y, double z) {
        this(EntityInit.ZOMBIE_HEAD.get(), level);
        setPos(x,y,z);

    }

    public ZombieHead(Level level, BlockPos position) {
    ...
    @Override
    public List<ExtendedSensor<ZombieHead>> getSensors() {
        return ObjectArrayList.of(
                new NearbyLivingEntitySensor<ZombieHead>().setRadius(20).setPredicate((target, entity) -> target instanceof Villager || target instanceof Player),// This tracks nearby entities
    ...
    @Override
    public BrainActivityGroup<ZombieHead> getCoreTasks() { // These are the tasks that run all the time (usually)
    ...
    @Override
    public BrainActivityGroup<ZombieHead> getIdleTasks() { // These are the tasks that run when the mob isn't doing anything else (usually)
        return BrainActivityGroup.idleTasks(
                new FirstApplicableBehaviour<ZombieHead>(      // Run only one of the below behaviours, trying each one in order. Include the generic type because JavaC is silly
    ...
    @Override
    public BrainActivityGroup<ZombieHead> getFightTasks() { // These are the tasks that handle fighting
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 5).add(Attributes.MOVEMENT_SPEED, 0.25f).add(Attributes.FOLLOW_RANGE, 30.0D).add(Attributes.ATTACK_DAMAGE, 2);
    }

    public static boolean canSpawn(EntityType<ZombieHead> entityType, ServerLevelAccessor level, MobSpawnType spawnType, BlockPos position, RandomSource random) {
